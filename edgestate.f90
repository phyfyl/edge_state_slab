  subroutine edgestat
     ...
     real(dp) :: emin, emax, w, k, time_start, time_end, s0(3), s1(3)
    ...
    !ENDDO
     ...
     ALLOCATE( sigma_x(nslab*ndim,nslab*ndim), sigma_y(nslab*ndim,nslab*ndim),&
               sigma_z(nslab*ndim,nslab*ndim), ctemp(nslab*ndim,nslab*ndim))
     ...
     allocate(GLL(nslab*Ndim, nslab*Ndim), GRR(nslab*Ndim, nslab*Ndim),&
              GB (nslab*Ndim, nslab*Ndim))
     allocate(H00(nslab*Ndim, nslab*Ndim), H01(nslab*Ndim, nslab*Ndim),&
              ones(nslab*Ndim, nslab*Ndim))
     ...
     do ikp= 1+cpuid, knv2, num_cpu
        ...
        call ham_qlayer2qlayerribbon2(k,H00,H01)
        ...
        do j = 1, omeganum
           ...
           ! subroutine
           call surfgreen_edge(w,GLL,GRR,GB,H00,H01,ones)
           ...
           do i= 1, NslabBottomOrbitals
              io= nslab*Ndim- nslab*Num_wann+ SlabBottomOrbitals(i)
           ...
           enddo ! i
            ...
            call mat_mul(nslab*ndim,gll,sigma_x,ctemp)
            do i = 1, NSlabtopOrbitals
                io = SlabTopOrbitals(i)
                sx_l_mpi(ikp, j) = sx_l_mpi(ikp, j)- AIMAG(ctemp(io,io))
            enddo ! i
            call mat_mul(nslab*ndim,gll,sigma_y,ctemp)
            do i = 1, NSlabtopOrbitals
                io = SlabTopOrbitals(i)
                sy_l_mpi(ikp, j) = sy_l_mpi(ikp, j)- AIMAG(ctemp(io,io))
            enddo !
            call mat_mul(nslab*ndim,gll,sigma_z,ctemp)
            do i = 1, NSlabtopOrbitals
                io = SLabTopOrbitals(i)
                sz_l_mpi(ikp, j) = sz_l_mpi(ikp, j)- AIMAG(ctemp(io,io))
            enddo ! i
            call mat_mul(nslab*ndim,grr,sigma_x,ctemp)
            do i = 1, NSlabtopOrbitals
                io = SlabTopOrbitals(i)
                sx_r_mpi(ikp, j) = sx_r_mpi(ikp, j)- AIMAG(ctemp(io,io))
            enddo ! i
            call mat_mul(nslab*ndim,grr,sigma_y,ctemp)
            do i = 1, NSlabtopOrbitals
                io = SlabTopOrbitals(i)
                sy_r_mpi(ikp, j) = sy_r_mpi(ikp, j)- AIMAG(ctemp(io,io))
            enddo !
            call mat_mul(nslab*ndim,grr,sigma_z,ctemp)
            do i = 1, NSLabtopOrbitals
                io = SlabTopOrbitals(i)
